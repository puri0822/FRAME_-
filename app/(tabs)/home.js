...
import { getIngredients, addToFridge } from "../store";
...
function SettingsModal({ visible, onClose }) {
  ...
  async function saveNickname() {
    ...
    if (!trimmed || !uid) { console.warn("saveNickname: trimmed=", trimmed, "uid=", uid, "user=", JSON.stringify(user)); return; }
    setNickSaving(true);
    try {
      ...
      else { console.error("닉네임 저장 실패 응답:", res.status, await res.text()); Alert.alert("저장 실패", "닉네임 저장에 실패했어요. 다시 시도해주세요."); }
    } catch (e) { console.error("닉네임 저장 오류:", e); Alert.alert("저장 실패", "서버 연결에 실패했어요."); } finally {
      setNickSaving(false);
    }
  }
  ...
  return (
    <Modal visible={visible} transparent animationType="slide" onRequestClose={onClose}>
      ...
      <Pressable style={st.modalOverlay} onPress={onClose}>
        <Pressable style={st.settingsSheet} onPress={() => {}}>
