...
import { setIngredients, loadFridgeFromStorage, saveFridgeToStorage, setFridgeAddCallback, triggerExploreSearch } from "../store";
...
function CatalogDetailSheet({ item, onClose, onAdd }) {
  ...
  return (
    <Pressable style={{ position: "absolute", top: 0, left: 0, right: 0, bottom: 0, backgroundColor: "rgba(0,0,0,0.4)", justifyContent: "flex-end" }} onPress={onClose}>
...
export default function FridgeScreen() {
  ...
  // 챗봇에서 냉장고 추가 요청 콜백 등록
  useEffect(() => {
    setFridgeAddCallback((chatItems) => {
      setItems((prev) => {
        const toAdd = chatItems
          .filter(ci => ci.name?.trim() && !prev.some(p => p.name === ci.name.trim()))
          .map(ci => ({ id: ++_idSeq, name: ci.name.trim(), category: ci.category || "채소/과일", count: ci.count || 1, expiry: ci.expiry || "" }));
        return toAdd.length > 0 ? [...prev, ...toAdd] : prev;
      });
    });
    return () => setFridgeAddCallback(null);
  }, []);
  ...
  return (
    <View style={[st.container, { paddingTop: (Constants.statusBarHeight || 0) + 12 }]}>
      ...
      {selectedIds.size > 0 && (
        <TouchableOpacity
          style={[st.recipeSearchBtn, { margin: 12, marginTop: 8 }]}
          onPress={() => {
            triggerExploreSearch(selectedNames.join(" "));
