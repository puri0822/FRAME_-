  ...
  try {
    ...
    if (parsed.intent === 'FRIDGE_SAVE' && Array.isArray(parsed.items) && parsed.items.length > 0) {
      ...
      if (recipes.length < 3) {
        ...
        const placeholder = excludeIds.length > 0 ? `AND r.id NOT IN (${excludeIds.join(',')}) ` : '';
        const [extra] = await db.query(`
          SELECT r.id, r.name, r.image_url, r.cook_time_min, r.difficulty,
                 (SELECT JSON_ARRAYAGG(ingredient_name)
                  FROM recipe_ingredient WHERE recipe_id = r.id AND is_key = 1) AS key_ingredients
          FROM recipe r WHERE r.is_published = 1 ${placeholder}ORDER BY RAND() LIMIT ${3 - recipes.length}
        `);
        recipes = [...recipes, ...extra];
      }
      result.action = { type: 'RECIPE_SEARCH', recipes, query: parsed.query };
    }
