...
const Explore = (() => {
  ...
  function buildTrendCard(trend, rank) {
    ...
    countBadge.innerHTML = `🍳 현재 <strong>${trend.count}명</strong> 요리 중`;
    ...
    info.append(name, countBadge, tagRow);
    card.append(visual, info);
    card.addEventListener('click', () => RecipeModal.open(recipe.id));
    return card;
  }
  ...
  return { init, refreshRecos, toggleFavorite, isFavorite: id => favorites.has(id) };
})();
...
const Home = (() => {
  ...
  function init() {
    ...
    input?.addEventListener('input', () => {
      input.value.length > 0 ? hideChips() : showChips();
    });
